...
/******************************************************************************/
Anubis::Common::UUID::UUID(bool isNull)
{
  /* Check if the UUID should be nulled or a UUID generated. */
  if(isNull)
  {
    /* Null (0) all the octets in the UUID. */
    memset(fOctets, 0, kOctetCount);
  }
  else
  {
